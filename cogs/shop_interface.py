...
class ShopInterface(commands.Cog):
    ...
    async def edit_my_role(self, interaction: discord.Interaction):
        # ko the defer() roi send_modal()
        custom_role_data = db.get_custom_role(interaction.user.id, interaction.guild.id)
        if not custom_role_data:
            await interaction.response.send_message("<a:c_947079524435247135:1274398161200484446> Bạn chưa sở hữu role tùy chỉnh nào để chỉnh sửa.", ephemeral=True)
            return
        ...
        if not role_obj:
            ...
            await interaction.response.send_message("<a:c_947079524435247135:1274398161200484446> Role tùy chỉnh của bạn không còn tồn tại trên server. Dữ liệu đã được xóa.", ephemeral=True)
            return
